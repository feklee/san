...
The fitness of an individual is calculated as a sum of deviations. The
lower the value, the better. For each node up to four deviations are
calculated, depending on how many neighbors it has.

What follows is an example of the calculation of the deviations for a
node with a full four neighbors.
...
The sum of this value for all the nodes is the fitness.
