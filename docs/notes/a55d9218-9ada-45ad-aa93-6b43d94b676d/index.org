...
Each node has several child nodes and one parent node. By default each
node acts as an oscillator possibly at a base frequency detuned
depending on the node’s position. All oscillators are simply
added. This makes the structure make sound by default without any user
interaction.
...
If possible, it should be possible to send signals around loops that
users built.

The UI for a node could be a webapp compatible with smartphones.

#+CAPTION: Module mockup
[[./images/module.png]]
